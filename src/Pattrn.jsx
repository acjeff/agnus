...
import { PALETTES, buildColorMap } from "./constants/palettes.js";
import { PUZZLE_THEMES, isThemeUnlocked } from "./constants/puzzleThemes.js";
import { ACHIEVEMENTS, SOLVE_MODES, computeAchievements } from "./constants/achievements.js";
import { DIFFICULTIES, COOP_MODES, MODE_CATEGORIES, CATEGORY_ICONS } from "./constants/difficulties.js";

import {
  rng, shuffle, getDateString, getDailySeedForIndex, getDailyDateLabel,
...
import {
  ...
  loadActiveCosmetic, saveActiveCosmetic, loadTheme, saveTheme,
  loadSavedAchievements, saveSavedAchievements,
  loadProgress, saveProgress, loadTimes, saveTimes,
} from "./utils/storage.js";
import {
  formatTime, solutionFillsFromPuzzle, parseToken, getShapeStroke,
  countModeSolved,
} from "./utils/helpers.js";
import { getSearchParams, updateUrl, setCoopUrlParam, clearCoopUrlParam } from "./utils/url.js";
import { friendlyAuthError } from "./utils/auth.js";

import {
  AGGIE_ID, AGGIE_LABEL, AGGIE_SIZES, ACCESSORY_CATEGORIES, AGGIE_ACCESSORIES,
  AGGIE_MAX_HAPPINESS, COINS_REWARD, COINS_GOLD_BONUS,
  getHappinessMood, AGGIE_SHOP_ITEMS, AGGIE_TRAITS, ROOM_ITEM_POSITIONS,
  AGGIE_LAST_STREAK_KEY, AGGIE_PUZZLE_DESIRES,
} from "./aggie/constants.js";
import {
  loadUnlockedAccessories, saveUnlockedAccessories,
  loadAggieAccessory, saveAggieAccessory, getActiveAccessoryIds,
  loadAggieSize, saveAggieSize,
  loadAggieCoins, saveAggieCoins,
  loadAggieHappiness, saveAggieHappiness,
  loadAggieLastInteract, saveAggieLastInteract,
  loadAggieInventory, saveAggieInventory,
  loadAggieDesire, saveAggieDesire, pickNewDesire,
  loadAggieBuff, saveAggieBuff,
  loadAggieDebuff, saveAggieDebuff,
  loadFailStreak, saveFailStreak,
  loadAggieTraits, saveAggieTraits, hasTraitEffect,
  calcDecayedHappiness,
} from "./aggie/state.js";
import { renderAggieSVG, renderAccessoryPreview, renderRoomItemIcon } from "./aggie/renderer.jsx";
import {
...
} from "./aggie/speech.js";

import DraggableDrawer from "./components/DraggableDrawer.jsx";
...
export default function Pattrn() {
  ...
  const [aggieWardrobeTab, setAggieWardrobeTab] = useState("shop"); // "shop" | "accessories" | "hangout"
  ...
  const renderContextButton = (currentView, pillButtons = [], bottomPx = 16) => {
    ...
    const friendsContentHeight = (() => {
      ...
      const screenH = typeof window !== "undefined" ? window.innerHeight : 700;
      const availH = screenH - bottomPx - 40; // leave margin top + bottom
      let h = panelPad + fabSize; // padding + bottom bar
      if (friendChatOpen) {
        // Chat view: header + messages area + input — fill available space
        h += 56 + 8; // header with back button + friend info + gap
        h += Math.max(300, availH - 200) + 8; // messages area (large) + gap
        h += 44; // input row
        h += 12; // bottom padding
      } else {
        h += 28 + 12; // header + margin
        h += 44 + 8; // add friend input + margin
        h += Math.min(friendsList.length, 8) * 72 + 16; // friend list items (show up to 8, rest scrolls)
        h += 12; // bottom padding
      }
      return h;
    })();
    ...
    const contentHeight = isAchievementsView ? achievementsContentHeight :
                          ...
                          isVaultChat ? (() => { const screenH = typeof window !== "undefined" ? window.innerHeight : 700; return panelPad + fabSize + 56 + 8 + Math.max(300, screenH - bottomPx - 240) + 8 + 44 + 12; })() :
                          (visibleItemCount * itemHeight + (showDivider ? dividerHeight : 0) + panelPad + fabSize + passUIHeight);
    // Cap panel height at 85vh
                    ...
                    <>
                        ...
                        <div style={{ display: "flex", gap: 8, marginBottom: addFriendMsg ? 4 : 12, flexShrink: 0, padding: "0 4px" }}>
                          ...
                          <button
                            ...
                            style={{
                              width: 38, height: 38, borderRadius: "50%", fontSize: 10, fontWeight: 700,
                              fontFamily: "'Inter', sans-serif",
                              background: addFriendInput.trim() ? C.accent : C.textDim + "22",
                          ...
                          >
                            {addFriendLoading ? "..." : <UserPlus size={16} strokeWidth={2.5} />}
                          </button>
                        </div>
                        {addFriendMsg && (
                          <div style={{ fontSize: 11, color: addFriendMsg.includes("added") ? C.correct : C.textDim, marginBottom: 8, fontFamily: "'Inter', sans-serif", padding: "0 4px" }}>
                            {addFriendMsg}
                          </div>
                        )}
                        ...
                        <div style={{ flex: 1, minHeight: 0, overflowY: "auto", display: "flex", flexDirection: "column", gap: 2 }}>
                          {friendsList.length === 0 ? (
                            <div style={{ textAlign: "center", padding: "40px 20px", color: C.textDim, fontSize: 13, fontFamily: "'Inter', sans-serif" }}>
                              <div style={{ fontSize: 28, marginBottom: 8, opacity: 0.4 }}>
                                <Users size={28} strokeWidth={1.5} color={C.textDim} />
                              </div>
                              No friends yet
                              <div style={{ fontSize: 11, marginTop: 4, opacity: 0.7 }}>Add someone above to get started</div>
                            </div>
                          ) : (
                            (() => {
                              ...
                              return sortedFriends.map(friend => {
                                ...
                                if (isPlaying) {
                                  ...
                                  statusText = currentSession ? `Playing ${modeName} (Co-op)` : `Playing ${modeName}`;
                                  ...
                                  <div key={friend.uid}>
                                    {showingConfirm ? (
                                      <div style={{
                                        ...
                                        border: `1px solid ${C.incorrect}66`,
                                        backgroundColor: C.incorrect + "08",
                                        transition: "all 0.2s",
                                      }}>
                                        <div style={{
                                          fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 10,
                                        }}>
                                        ...
                                        <div style={{ display: "flex", gap: 8 }}>
                                          <button
                                            ...
                                            style={{
                                              ...
                                              backgroundColor: "rgba(255,255,255,0.06)", border: `1px solid ${C.border}`, color: C.text,
                                          ...
                                          <button
                                            ...
                                            style={{
                                              ...
                                              fontSize: 11, fontWeight: 700, fontFamily: "'Inter', sans-serif",
                                              textTransform: "uppercase", letterSpacing: 0.5,
                                              backgroundColor: C.incorrect, border: "none", color: "#fff",
                                              cursor: "pointer", transition: "all 0.15s",
                                            }}
                                          >
                                            Remove
                                      ...
                                      /* Conversation row — tap anywhere to open chat */
                                      <div
                                        onClick={openChat}
                                        style={{
                                          display: "flex", alignItems: "center", gap: 12,
                                      ...
                                      >
                                        ...
                                        <div style={{ position: "relative", flexShrink: 0 }}>
                                          {friend.profilePicture ? (
                                            <img src={friend.profilePicture} alt="" style={{ width: 44, height: 44, borderRadius: "50%", objectFit: "cover" }} />
                                          ) : (
                                            <div style={{ width: 44, height: 44, borderRadius: "50%", backgroundColor: C.accent + "22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, color: C.accent, fontWeight: 700 }}>
                                          ...
                                          <div style={{
                                            ...
                                            border: `2.5px solid ${C.surface || "#1a1a2e"}`,
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                                            <span style={{
                                              fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: hasUnread ? 700 : 600, color: C.text,
                                          ...
                                          {/* Last message preview or activity status */}
                                          {chatMeta && chatMeta.lastMessage ? (
                                            <div style={{
                                              fontSize: 12, color: hasUnread ? C.text : C.textDim,
                                              fontFamily: "'Inter', sans-serif",
                                            ...
                                            <div style={{ fontSize: 12, color: statusColor, fontFamily: "'Inter', sans-serif", lineHeight: 1.3 }}>
                                              {statusText}
                                            </div>
                                          )}
                                        </div>

                                        {/* Right side: time + unread badge + actions */}
                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
                                          {chatMeta && chatMeta.lastMessageAt && (
                                            <span style={{ fontSize: 10, color: hasUnread ? C.accent : C.textDim, fontFamily: "'Inter', sans-serif", fontWeight: hasUnread ? 600 : 400 }}>
                                              {fmtMsgTime(chatMeta.lastMessageAt)}
                                            </span>
                                          )}
                                          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                                            ...
                                            {currentSession && (
                                              <button onClick={(e) => {
                                                ...
                                                if (currentSession && presence.currentMode && presence.currentPuzzle) {
                                                  ...
                                                  setDifficulty(mode);
                                                  if (mode === "daily") { setDailyDate(puzzleId); setCurrentDailyDate(puzzleId); }
                                                  else if (mode === "cascade") { setCascadeRunIndex(parseInt(puzzleId) || 0); }
                                                  else { setCurrentPuzzle(parseInt(puzzleId) || 0); }
                                                  setRadialMenuStack(["root"]);
                                                  if (firebaseUser && username) joinCoopSession(currentSession, firebaseUser.uid, username).catch(() => {});
                                                }
                                              }} style={{
                                            ...
                                            <button
                                              onClick={(e) => { e.stopPropagation(); setRemoveFriendConfirm(friend.uid); }}
                                              title="Remove friend"
                                              style={{
                                                width: 22, height: 22, borderRadius: 6,
                                                backgroundColor: "transparent", border: "none",
                                                color: C.textDim + "66", cursor: "pointer",
                                                display: "flex", alignItems: "center", justifyContent: "center",
                                                transition: "all 0.15s", flexShrink: 0, padding: 0,
                                                opacity: 0,
                                              }}
                                              onMouseEnter={e => { e.currentTarget.style.color = C.incorrect; e.currentTarget.style.opacity = 1; }}
                                              onMouseLeave={e => { e.currentTarget.style.color = C.textDim + "66"; e.currentTarget.style.opacity = 0; }}
                                            >
                          ...
                          return (
                            <button key={key} onClick={() => setAggieWardrobeTab(key)}
                              style={{
                                ...
                                cursor: "pointer", transition: "all 0.15s",
                                borderRight: tabIdx < 2 ? `1px solid ${C.border}` : "none",
                                position: "relative",
                              }}
                            >
                              {label}
                        ...
                        {aggieWardrobeTab === "hangout" && (
                          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            ...
                            {hangoutSharedTraits.filter(tId => !aggieTraits.includes(tId)).length > 0 && (
                              <div>
                                <div style={{ fontSize: 10, fontWeight: 700, color: C.correct, fontFamily: "'Inter', sans-serif", marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>
                                ...
                                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                                  {hangoutSharedTraits.filter(tId => !aggieTraits.includes(tId)).map(tId => {
                                    ...
                                    if (!fromPeer) {
                                      for (const oh of offlineHangouts) {
                                        const participants = oh.participants || {};
                            ...
                            <div>
                              <div style={{ fontSize: 10, fontWeight: 700, color: C.text, fontFamily: "'Inter', sans-serif", marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>
                                Friends
                              </div>
                              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                                {friendsList.length === 0 ? (
                                  <div style={{ textAlign: "center", padding: 16, color: C.textDim, fontSize: 11, fontFamily: "'Inter', sans-serif" }}>
                                  ...
                                  return (
                                    <div key={friend.uid} style={{
                                      ...
                                      <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: 11, fontWeight: 600, color: C.text, fontFamily: "'Inter', sans-serif" }}>
                                          {friend.username || "Friend"}
                                        </div>
                                        <div style={{ fontSize: 8, color: isHanging ? C.correct : C.textDim, fontFamily: "'Inter', sans-serif" }}>
                                      ...
                                      {hangoutActive && !isPeerInHangout && !isOnOfflineHangout && isOnline && (
                                        <span style={{ fontSize: 8, color: C.textDim, fontFamily: "'Inter', sans-serif", opacity: 0.5 }}>
                                          Waiting...
                                        </span>
                                      )}
      ...
      {gameState === "won" && showWinOverlay && (
        <div style={{
          ...
          <div style={{ fontSize: 26, fontWeight: 700, fontFamily: "'Inter', sans-serif", color: C.correct, animation: "fadeUp 0.5s 0.05s ease both" }}>
            {isCoop ? "Co-op complete!" : isCascade ? (cascadeLevel >= CASCADE_LEVELS.length - 1 ? "Cascade complete!" : "Level clear!") : isBlind ? "Cracked it!" : isSpin ? "Nailed it!" : isMosaic ? "Tile complete!" : (attempts <= 1 ? "Perfect!" : attempts === 2 ? "Brilliant!" : attempts === 3 ? "Great!" : attempts === 4 ? "Not bad!" : "Solved!")}
          </div>
          {isCoop && (
